...
namespace Accounting.FSharp.Control.Tasks
...
module TaskBuilder =
    ...
    and StepStateMachine<'a>(firstStep) as this =
        ...
        interface IAsyncStateMachine with
            ...
            /// If awaiting, MoveNext() will be called again when the awaitable complete.
            member _.MoveNext() =
            ...
            member _.SetStateMachine _ = () // Doesn't really apply since we're a reference type.
        ...
        | ReturnFrom t ->
            ...
            Await(
                ...
                fun () ->
                    ...
                    fin () // if we got here we haven't run fin(), because we would've re-raised after doing so
                    result
            )
    ...
    let run (firstStep: unit -> Step<'a>) =
        ...
        with exn ->
            let src = TaskCompletionSource<_>()
    ...
    type TaskBuilderV2() =
        ...
        member _.Delay(f: unit -> Step<_>) = f
        member _.Run(f: unit -> Step<'m>) = run f
        member _.Zero() = zero
        member _.Return(x) = ret x
        member _.Combine(step: unit Step, continuation) = combine step continuation
        member _.While(condition: unit -> bool, body: unit -> unit Step) = whileLoop condition body
        member _.For(sequence: _ seq, body: _ -> unit Step) = forLoop sequence body
        member _.TryWith(body: unit -> _ Step, catch: exn -> _ Step) = tryWith body catch
        member _.TryFinally(body: unit -> _ Step, fin: unit -> unit) = tryFinally body fin
        member _.Using(disp: #IDisposable, body: #IDisposable -> _ Step) = using disp body
        member _.ReturnFrom a : _ Step = ReturnFrom a
    ...
    type TaskBuilder() =
        ...
        member inline _.Delay(f: unit -> Step<_>) = f
        member inline _.Run(f: unit -> Step<'m>) = run f
        member inline _.Zero() = zero
        member inline _.Return(x) = ret x
        member inline _.Combine(step: unit Step, continuation) = combine step continuation
        member inline _.While(condition: unit -> bool, body: unit -> unit Step) = whileLoop condition body
        member inline _.For(sequence: _ seq, body: _ -> unit Step) = forLoop sequence body
        member inline _.TryWith(body: unit -> _ Step, catch: exn -> _ Step) = tryWith body catch
        member inline _.TryFinally(body: unit -> _ Step, fin: unit -> unit) = tryFinally body fin
        member inline _.Using(disp: #IDisposable, body: #IDisposable -> _ Step) = using disp body
        ...
        member inline _.ReturnFrom(task: _ Task) = ReturnFrom task
        member inline _.Bind(task: 'a Task, continuation: 'a -> 'b Step) : 'b Step = bindTask task continuation
    ...
    type ContextInsensitiveTaskBuilder() =
        ...
        member inline _.Bind(task: 'a Task, continuation: 'a -> 'b Step) : 'b Step =
            bindTaskConfigureFalse task continuation


// Don't warn about our use of the "obsolete" module we just defined (see notes at the start of the file).
#nowarn "44"
...
module ContextSensitive =
    ...
    type TaskBuilder.TaskBuilder with
        ...
        member _.ReturnFrom(a: 'a Async) =
            TaskBuilder.bindTask (Async.StartAsTask a) TaskBuilder.ret

        member _.Bind(a: 'a Async, continuation: 'a -> 'b TaskBuilder.Step) : 'b TaskBuilder.Step =
...
module V2 =
    ...
    module ContextSensitive =
        ...
        type TaskBuilderV2 with
            ...
            member inline _.Bind(task, continuation: 'a -> 'b Step) : 'b Step = (BindS.Priority1 >>= task) continuation
            member inline _.ReturnFrom a : 'b Step = ReturnFromS.Priority1 $ a
    ...
    module ContextInsensitive =
        ...
        type TaskBuilderV2 with
            ...
            member inline _.Bind(task, continuation: 'a -> 'b Step) : 'b Step = (BindI.Priority1 >>= task) continuation
            member inline _.ReturnFrom a : 'b Step = ReturnFromI.Priority1 $ a
